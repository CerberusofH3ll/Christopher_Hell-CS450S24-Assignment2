...
def cornersHeuristic(state, problem):
    ...
    for perm in permutations(unvisitedCorners):
        current_cost = 0
        current_position = position
        for corner in perm:
            current_cost += util.manhattanDistance(current_position, corner)
            current_position = corner
        if current_cost < min_additional_cost:
            min_additional_cost = current_cost
    
    heuristic += min_additional_cost
